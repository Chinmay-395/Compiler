...
let better_layout (f:Ll.fdecl) (live:liveness) : layout =
  ...
  *)
  ...
  in
  (* the code *)
  let highest_degree_node (degs:int UidM.t) : (uid*int) option = 
    let check_uid = (fun uid deg t ->
      match t with
      | None -> Some (uid, deg)
      | Some (new_uid, new_deg) -> if deg < new_deg 
                            then Some (new_uid, new_deg)
                            else Some (uid, deg) 
    ) in
    UidM.fold check_uid degs None in
  (* Step 1: Find a node with degree < K and cut it out of the graph.
    ...
    – Step 3: When remaining graph is colored, there must be at least one free
    color available for the deleted node (since its degree was < K). Pick such a
    color. 
  *)
  let rec kempe_algo (graph : (UidS.t ref) UidM.t) (n:int) 
    (pre_coloring:(Ll.uid * Alloc.loc) list) 
    (choosen_colors:(Ll.uid * LocSet.t) list) : (Ll.uid * Alloc.loc) list =
    if UidM.cardinal graph = 0 
      then [] 
    else
      let non_precolored = UidM.filter (fun u _ -> not (List.mem_assoc u pre_coloring)) graph in
      if UidM.cardinal non_precolored = 0 then
        (* All nodes pre-colored *)
        pre_coloring
      else
    let non_precolored_degs = UidM.map (fun sref -> UidS.cardinal !sref) non_precolored in
    let the_node = 
      (* If there are k registers available and m > k temporaries are live at the same
        time, then not all of them will fit into registers *)
      let find = highest_degree_node @@ UidM.filter (fun _ _val -> _val < n) non_precolored_degs in
      match find with
      | None -> 
        begin 
          match (highest_degree_node non_precolored_degs) with
            | None -> failwith "Cannot find node to spill"
            | Some (the_node,_) -> 
              the_node
        end
      | Some (the_node, _) -> 
        the_node
    in
    let new_graph = UidM.remove the_node graph in
    UidM.iter (fun _ sref -> sref := UidS.remove the_node !sref) new_graph;
    let coloring = kempe_algo new_graph n pre_coloring choosen_colors in
    let finding_ele_list = (UidS.elements !(UidM.find the_node graph)) in
    let neighbouring_node_colors = List.map (fun x -> List.assoc x coloring) finding_ele_list in
    let available_colors = LocSet.filter (fun x -> not (List.mem x neighbouring_node_colors)) pal in
    let choosen_node = List.assoc_opt the_node choosen_colors in
    let color_opt = 
      match choosen_node with
        | Some pref_colors -> 
          LocSet.choose_opt @@ LocSet.inter available_colors pref_colors
        | None -> None
    in
    match color_opt with
      | Some x -> (the_node, x) :: coloring
      | None -> begin 
                  match LocSet.choose_opt available_colors with
                    | None -> (the_node, spill()) :: coloring
                    | Some x -> (the_node,x) :: coloring
                end
    in
  ...
  ignore @@ fold_fdecl
    ...
    (fun _ (x,i) ->
      ...
      with Not_found -> failwith "uid's not found in graph"
    )
    (fun _ _ -> ())
    () f;
  ...
  let coloring : (Ll.uid * Alloc.loc) list = kempe_algo interference_graph k pre_coloring (UidM.bindings preferred_colors) in (* find k-coloring, try to keep parameters assigned *)
