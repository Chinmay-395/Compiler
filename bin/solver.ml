...
module Make (Fact : FACT) (Graph : DFA_GRAPH with type fact := Fact.t) =
  struct
    ...
    let solve (g:Graph.t) : Graph.t =
      let ref_node= ref @@ Graph.nodes g in
let graph= ref g in
while not @@ Graph.NodeS.is_empty !ref_node do
let nth_node= Graph.NodeS.choose !ref_node in
ref_node:= Graph.NodeS.remove nth_node !ref_node;
let old= Graph.out !graph nth_node in
let new_= Graph.preds !graph nth_node |>
Graph.NodeS.elements |>
List.map( Graph.out !graph) |>
Fact.combine |>
Graph.flow !graph nth_node in
graph:= Graph.add_fact nth_node new_ !graph;
let successor= Graph.succs !graph nth_node in
if Fact.compare old (Graph.out !graph nth_node)!= 0 then
ref_node:= Graph.NodeS.union !ref_node successor
done;
!graph
